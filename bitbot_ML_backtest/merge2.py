...
def calculate_profit(data, risk_reward_ratio, envelope_period, std_multiplier):
    ...
    max_consecutive_same_position_losses = 0

    for i in range(len(data)):
        if position == 0:
            if data['cross_upper'].iloc[i] == 1 and not (last_position == -1 ):
                position = -1
            ...
            elif data['cross_lower'].iloc[i] == 1 and not (last_position == 1 ):
                position = 1
                entry_price = data['close'].iloc[i]
            ...
            if data['low'].iloc[i] <= exit_price:
                ...
                if last_position == -1:
                    ...
                    consecutive_losses = 0
                max_consecutive_same_position_losses = max(max_consecutive_same_position_losses, consecutive_losses)
            elif data['high'].iloc[i] >= data['sma'].iloc[i]:
            ...
            if data['high'].iloc[i] >= exit_price:
                ...
                if last_position == 1:
                    ...
                    consecutive_losses = 0
                max_consecutive_same_position_losses = max(max_consecutive_same_position_losses, consecutive_losses)
            elif data['low'].iloc[i] <= data['sma'].iloc[i]:
                profit -= ((entry_price - data['close'].iloc[i]) / entry_price * 100) + trading_fee
                position = 0
                consecutive_losses = 0

    win_rate = 0 if total_trades == 0 else (consecutive_losses / total_trades) * 100

    return profit, total_trades, win_rate, max_consecutive_same_position_losses
...
def objective(params):
    ...
    profit, _, win_rate, _ = calculate_profit(data, risk_reward_ratio, envelope_period, std_multiplier)
...
space = {
    'risk_reward_ratio': hp.uniform('risk_reward_ratio', 1.1, 20),
    'envelope_period': hp.quniform('envelope_period', 2, 100, 1),
...
# profit, total_trades, win_rate, max_consecutive_same_position_losses = calculate_profit(data, 4.34, 2, 3.8)
# print(f'Profit: {profit}%')
# print(f'Total trades: {total_trades}')
# print(f'Win rate: {win_rate}%')
# print(f'Max consecutive same position losses: {max_consecutive_same_position_losses}')
